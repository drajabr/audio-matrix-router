...
public sealed class OutputSyncCoordinator
{
    ...
    // Above this phase error a trim-only walk-back takes tens of seconds (2000 ppm walks
    // 20 ms in ~10 s); do a one-shot hard re-anchor (skip or hold) instead.
    private const double HardResyncThresholdMs = 20.0;
    // Minimum interval between hard re-anchors per consumer, so a re-anchor in progress
    // (whose error is still being consumed) can't re-trigger itself.
    private const double HardResyncCooldownSec = 2.0;
    // The start barrier must never hold outputs silent forever (a capture that delivers
    // one frame then stalls used to pin the fill measurement at 0 with no way out).
    private const double BarrierWatchdogMinMs = 1500.0;
    ...
    private sealed class OutputState
    {
        ...
        // One-shot hard re-anchor handed to the provider: positive = skip that many
        // engine frames forward (we were behind), negative = hold consumption for that
        // many engine frames (we were ahead, e.g. after a master stall recovered).
        public double PendingHardResyncFrames;
        public long LastHardResyncTicks;
    }
    ...
    public void ArmGlobalRefillHold()
    {
        lock (_lock)
        {
            ...
            _barrierArmedTicks = Stopwatch.GetTimestamp();
            ResetAllNoLock();
        }
    }
    ...
    private void ResetAllNoLock()
    {
        foreach (var s in _states.Values)
        {
            ...
            s.PendingHardResyncFrames = 0;
            s.LastHardResyncTicks = 0;
        }
    }
    ...
    public bool ShouldHoldForGlobalRefill()
    {
        lock (_lock)
        {
            ...
            // Watchdog: a stalled capture (one frame then silence) or a dead ring must
            // not hold every output silent forever. Force-release after a bounded wait —
            // healthy consumers play; a starved one underruns audibly, which at least
            // surfaces the problem instead of masking it as total silence.
            long nowTicks = Stopwatch.GetTimestamp();
            if (_barrierArmedTicks != 0)
            {
                double elapsedMs = (nowTicks - _barrierArmedTicks) * 1000.0 / s_ticksPerSecond;
                double maxHoldTargetMs = 0;
                foreach (var st in _states.Values)
                {
                    double ms = st.HoldTargetFrames * 1000.0 / _engineSampleRate;
                    if (ms > maxHoldTargetMs) maxHoldTargetMs = ms;
                }
                double timeoutMs = Math.Max(BarrierWatchdogMinMs, 3 * maxHoldTargetMs);
                if (elapsedMs > timeoutMs)
                {
                    Debug.WriteLine($"[SyncCoordinator] Start barrier watchdog fired after {elapsedMs:F0} ms — forcing release");
                    ReleaseBarrierNoLock(nowTicks);
                    return false;
                }
            }
    ...
    private void ReleaseBarrierNoLock(long nowTicks)
    {
        _startBarrierActive = false;
        foreach (var s in _states.Values)
        {
            s.TimelinePos = 0;
            s.LastReportTicks = nowTicks;
            s.SmoothedErrorFrames = 0;
            s.ErrorInitialized = false;
            s.IntegralFrames = 0;
            s.TrimPpm = 0;
    ...
    /// <summary>
    /// Returns and clears any pending hard re-anchor for this consumer. Positive: skip
    /// that many engine frames of ring content forward. Negative: hold consumption for
    /// that many engine frames (render silence, timeline frozen).
    /// </summary>
    public double TakeHardResyncFrames(string consumerId)
    {
        lock (_lock)
        {
            if (!_states.TryGetValue(consumerId, out var s)) return 0;
            var pending = s.PendingHardResyncFrames;
            s.PendingHardResyncFrames = 0;
            return pending;
        }
    }
    ...
    public double ReportConsumedAndUpdate(string consumerId, double engineFramesConsumed, int deviceFramesRendered)
    {
        lock (_lock)
        {
            ...
            // Hard re-anchor for gross desyncs (master stall recovery, follower device
            // glitch): trim alone would need tens of seconds at 2000 ppm; one brief
            // skip/hold realigns immediately. Cooldown prevents re-triggering while the
            // provider is still consuming a previous re-anchor.
            double hardThresholdFrames = _engineSampleRate * HardResyncThresholdMs / 1000.0;
            if (Math.Abs(rawError) >= hardThresholdFrames)
            {
                double sinceLastSec = s.LastHardResyncTicks == 0
                    ? double.MaxValue
                    : (now - s.LastHardResyncTicks) / s_ticksPerSecond;
                if (sinceLastSec >= HardResyncCooldownSec)
                {
                    s.LastHardResyncTicks = now;
                    s.PendingHardResyncFrames += rawError;
                    if (rawError > 0)
                    {
                        // Behind: cursors will skip forward; timeline rejoins master now.
                        s.TimelinePos += rawError;
                    }
                    // Ahead (rawError < 0): the provider holds consumption while the
                    // master catches up; the timeline stays put, so no adjustment here.
                    s.SmoothedErrorFrames = 0;
                    s.ErrorInitialized = false;
                    s.IntegralFrames = 0;
                    s.TrimPpm = 0;
...
public class MixingSampleProvider : ISampleProvider
{
    ...
    // Engine frames of consumption to withhold (render silence, timeline frozen) after a
    // negative hard re-anchor — we ran ahead of the master and must let it catch up.
    private double _holdEngineFramesRemaining;
    ...
    public int Read(float[] buffer, int offset, int count)
    {
        ...
        // Hard re-anchor from the coordinator: positive → skip ring content forward
        // (we were behind the master), negative → hold consumption (we were ahead).
        double hardResync = _syncCoordinator.TakeHardResyncFrames(_consumerId);
        if (hardResync > 0.5)
        {
            long skipFrames = (long)Math.Round(hardResync);
            for (int i = 0; i < sourceCount; i++)
            {
                _pendingSkipPerSource[i] += skipFrames;
            }
        }
        else if (hardResync < -0.5)
        {
            _holdEngineFramesRemaining += -hardResync;
        }

        if (_holdEngineFramesRemaining > 0)
        {
            // Render silence without consuming or advancing the timeline; the master
            // catches up during the hold. Deliberately no ReportConsumedAndUpdate — the
            // frozen timeline would read as a fresh gross error and re-trigger.
            Array.Clear(buffer, offset, count);
            _holdEngineFramesRemaining -= frames * _baseRatio;
            if (_holdEngineFramesRemaining < 0) _holdEngineFramesRemaining = 0;
            _currentTrimPpm = 0;
            return count;
        }
        ...
        for (int srcIdx = 0; srcIdx < sourceCount; srcIdx++)
        {
            ...
            // Reconcile overflow trims: the ring may have force-advanced this consumer's
            // cursor while it lagged (producer never stalls). That advancement satisfies
            // pending skips first; any excess leaves the cursor AHEAD of the timeline
            // (negative pending), which step 3 pays back by consuming less. Without this
            // the phase controller reported zero error while outputs played different
            // content.
            long forced = src.Buffer.TakeForceAdvancedFrames(_consumerId);
            if (forced > 0)
            {
                _pendingSkipPerSource[srcIdx] -= forced;
            }
        ...
        {
            var src = _sources[srcIdx];
            int want = integerConsume;

            // Negative pending: the ring force-advanced this cursor past the timeline
            // (overflow trim). Pay it back by consuming less until they rejoin.
            long pend = _pendingSkipPerSource[srcIdx];
            if (pend < 0)
            {
                long payback = Math.Min(-pend, want);
                want -= (int)payback;
                _pendingSkipPerSource[srcIdx] = pend + payback;
            }

            int advance = Math.Min(want, _framesReadPerSource[srcIdx]);
            if (advance > 0)
            {
                src.Buffer.SkipForConsumer(_consumerId, advance);
            }
            int shortfall = want - advance;
            if (shortfall > 0)
            {
