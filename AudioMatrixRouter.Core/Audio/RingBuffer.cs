...
public class RingBuffer
{
    ...
    private sealed class ConsumerState
    {
        ...
        // Frames this cursor was force-advanced by overflow trimming since the consumer
        // last reconciled (TakeForceAdvancedFrames). The provider folds this into its
        // timeline bookkeeping so the phase controller never acts on a cursor that moved
        // behind its back.
        public long ForceAdvancedPending;
    }
    ...
    /// <summary>
    /// Returns and clears the frames this consumer's cursor was force-advanced by
    /// overflow trimming since the last call. The consumer uses this to reconcile its
    /// timeline with the cursor's actual position.
    /// </summary>
    public long TakeForceAdvancedFrames(string consumerId)
    {
        lock (_cursorLock)
        {
            if (!_consumers.TryGetValue(consumerId, out var c)) return 0;
            long pending = c.ForceAdvancedPending;
            c.ForceAdvancedPending = 0;
            return pending;
        }
    }
