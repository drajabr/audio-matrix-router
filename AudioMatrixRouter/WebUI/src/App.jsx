...
// Methods that legitimately take a long time on the host (engine restarts, downloads).
// Everything else gets the default timeout so a lost reply can never wedge the UI.
const BRIDGE_TIMEOUT_DEFAULT_MS = 10000;
const BRIDGE_TIMEOUT_BY_METHOD = {
  reloadEngine: 30000,
  setInputBufferMs: 30000,
  setOutputBufferMs: 30000,
  setCrosspoints: 30000,
  setCrosspoint: 30000,
  updateCheck: 30000,
  updateDownload: 0, // no timeout — download duration is unbounded; progress pushes cover liveness
};
...
function ensureNativeBridge() {
  ...
  window.__nativeBridgeInvoke = (method, params = {}) => {
    ...
    const timeoutMs = Object.prototype.hasOwnProperty.call(BRIDGE_TIMEOUT_BY_METHOD, method)
      ? BRIDGE_TIMEOUT_BY_METHOD[method]
  ...
  // Receive pushes from native. Uses PostWebMessageAsJson on the C# side, which
  // delivers a parsed object here. Three kinds:
  //   reply   — RPC responses (ordered with pushes, unlike the old ExecuteScriptAsync path)
  //   state   — routes/devices/config, sent only when something actually changed
  //   metrics — 10 Hz peaks/latency/counters; deliberately carries NO route state
  if (!window.__nativeBridgePushBound) {
    ...
    window.chrome.webview.addEventListener("message", (event) => {
      ...
      if (data.kind === "reply" && data.id != null) {
        window.__nativeBridgeResolve(data.id, data.result, data.error);
        return;
      }

      if (data.kind === "state" && data.state) {
        // Merge cached available-device lists when this push omitted them.
      ...
      if (data.kind === "metrics" && data.metrics) {
        window.dispatchEvent(new CustomEvent("native-metrics", { detail: data.metrics }));
        return;
      }

      if (data.kind === "update-progress") {
...
// Merges the persisted device ordering with the current on-screen ordering so that
// (a) the user's latest relative order of PRESENT devices wins, (b) devices that are
// currently offline keep their saved slots instead of being wiped, and (c) newly seen
// devices append at the end. Rebuilding order purely from the discovered list used to
// lose the position of any device that was unplugged.
function mergeOrder(savedOrder = [], currentIds = []) {
  ...
  saved.forEach((id) => {
    ...
    if (currentSet.has(id)) {
      // A slot that belongs to a present device: fill it with the next present device
      // in the CURRENT order, so live reordering still persists.
      while (cursor < current.length && seen.has(current[cursor])) cursor += 1;
      if (cursor < current.length) {
        const cid = current[cursor];
        cursor += 1;
        seen.add(cid);
        merged.push(cid);
      }
...
// One row of matrix tiles, memoized so the 10 Hz metrics stream (which only touches
// meter/header state) never re-renders the tile grid. Tiles carry NO event handlers —
// clicks/wheel/context-menu are delegated to the grid container via the data-rowid /
// data-colid attributes, which keeps every prop here a primitive or stable reference.
const MatrixRowTiles = React.memo(function MatrixRowTiles({
  row,
  rowIndex,
  cols,
  viewMode,
  ...
  selectedRowId,
  selectedColId,
  selectedRowPathIndex,
  selectedColPathIndex,
  scaledCellSize,
}) {
  const rowSpan = viewMode === "device" ? Math.max(1, row.channelCount || 1) : 1;
  return (
    <>
      {cols.map((col, colIndex) => {
        const key = getCellKey(row.id, col.id);
        const state = activeMatrix[key] || makeDefaultConnection();
        const selected = selectedRowId === row.id && selectedColId === col.id;
...
export default function App() {
  ...
  // Check-for-update button: idle → checking → available → downloading → ready.
  // "portable" means the build isn't Velopack-installed and can't self-update.
  ...
  // Rev gating: the host stamps every state snapshot with a monotonically increasing
  // rev. We only apply a snapshot that is strictly newer than the last one applied, and
  // never while one of our own mutations is in flight (its reply is authoritative).
  // This replaces the old fragile time-window suppression (localEditHoldUntilRef).
  const lastAppliedStateRevRef = useRef(0);
  const pendingMutationsRef = useRef(0);
  const nativeSyncVersionRef = useRef(0);
  const persistDebounceRef = useRef(0);
  const adjustGainRef = useRef(null);
  const persistedOrderRef = useRef({ inputs: [], outputs: [] });
  ...
  useEffect(() => {
    ...
    const applyNativeState = (state) => {
      ...
      // Rev gate: only apply snapshots strictly newer than the last applied one, and
      // never while one of our mutations is in flight (its reply — dispatched with
      // __authoritative — carries the newest truth). Stale/racing pushes can therefore
      // never overwrite an optimistic tile toggle.
      const authoritative = !!state.__authoritative;
      const rev = Number.isFinite(state?.rev) ? Number(state.rev) : null;
      if (rev != null) {
        if (rev < lastAppliedStateRevRef.current - 500) {
          // Host restarted and its rev counter reset.
          lastAppliedStateRevRef.current = 0;
        }
        if (!authoritative) {
          if (pendingMutationsRef.current > 0) return;
          if (rev <= lastAppliedStateRevRef.current) return;
        }
        lastAppliedStateRevRef.current = Math.max(lastAppliedStateRevRef.current, rev);
      }
      ...
      // Device identity meta (offsets/channels/rates) comes from state pushes; the
      // metrics stream merges peaks/counters into the same map without touching these.
      const inMeta = { ...nativeInputChannelMetaRef.current };
      (Array.isArray(state?.inputs) ? state.inputs : []).forEach((d) => {
        if (!d?.deviceId) return;
        const prev = inMeta[d.deviceId] || {};
        inMeta[d.deviceId] = {
          ...prev,
          ...
          peakLevels: Array.isArray(prev.peakLevels) ? prev.peakLevels : [],
        };
      });
      nativeInputChannelMetaRef.current = inMeta;
      setNativeInputChannelMeta(inMeta);

      const outMeta = { ...nativeOutputChannelMetaRef.current };
      (Array.isArray(state?.outputs) ? state.outputs : []).forEach((d) => {
        if (!d?.deviceId) return;
        const prev = outMeta[d.deviceId] || {};
        outMeta[d.deviceId] = {
          ...prev,
          offset: Number.isFinite(d?.offset) ? d.offset : 0,
          channels: Number.isFinite(d?.channels) ? d.channels : 0,
          sampleRate: Number.isFinite(d?.sampleRate) ? d.sampleRate : 0,
          driverLatencyMs: Number.isFinite(d?.driverLatencyMs) ? d.driverLatencyMs : 0,
          latencyMs: Number.isFinite(d?.latencyMs) ? Math.round(Number(d.latencyMs) * 10) / 10 : (prev.latencyMs ?? null),
          peakLevels: Array.isArray(prev.peakLevels) ? prev.peakLevels : [],
        };
      });
    ...
    // 10 Hz telemetry: peaks, latencies, jitter, counters. Never touches the matrix,
    // masters or device lists, so it cannot fight a route edit by construction.
    const applyNativeMetrics = (metrics) => {
      if (!metrics || cancelled) return;

      const total = Number.isFinite(metrics?.totalLatencyMs) ? Number(metrics.totalLatencyMs) : null;
      setNativeTotalLatencyMs(total != null ? Math.round(total * 10) / 10 : null);
      updateLatencyDisplay(total);

      const nativeInputPathLatency = Number.isFinite(metrics?.inputLatencyMs) ? Math.round(Number(metrics.inputLatencyMs) * 10) / 10 : null;
      const nativeOutputPathLatency = Number.isFinite(metrics?.outputLatencyMs) ? Math.round(Number(metrics.outputLatencyMs) * 10) / 10 : null;
      setInputLatencyMs(nativeInputPathLatency);
      setOutputLatencyMs(nativeOutputPathLatency);
      setBufferMs(nativeInputPathLatency);

      const nativeInputJitter = Number.isFinite(metrics?.inputJitterMs)
        ? Math.round(Number(metrics.inputJitterMs) * 10) / 10
        : null;
      ...
      const routeLatency = {};
      (Array.isArray(metrics?.routeLatencies) ? metrics.routeLatencies : []).forEach((route) => {
      ...
      setNativeRouteLatencyByCh((prev) => {
        const prevKeys = Object.keys(prev);
        const nextKeys = Object.keys(routeLatency);
        if (prevKeys.length === nextKeys.length && nextKeys.every((k) => prev[k] === routeLatency[k])) {
          return prev;
        }
        return routeLatency;
      });
      ...
      const inMeta = { ...nativeInputChannelMetaRef.current };
      (Array.isArray(metrics?.inputs) ? metrics.inputs : []).forEach((d) => {
        if (!d?.deviceId) return;
        const prev = inMeta[d.deviceId] || {};
        inMeta[d.deviceId] = {
          ...prev,
          ...
          peakLevels: Array.isArray(d?.peakLevels)
      ...
      const outMeta = { ...nativeOutputChannelMetaRef.current };
      (Array.isArray(metrics?.outputs) ? metrics.outputs : []).forEach((d) => {
        if (!d?.deviceId) return;
        const prev = outMeta[d.deviceId] || {};
        outMeta[d.deviceId] = {
          ...prev,
    ...
    const onNativeMetrics = (event) => applyNativeMetrics(event?.detail);
    if (hasNativeBridge) {
      window.addEventListener("native-state", onNativeState);
      window.addEventListener("native-metrics", onNativeMetrics);
    }

    void poll();
    // Native: state pushes are event-driven and metrics arrive at 10 Hz; the 1s poll is
    // only a watchdog for missed state pushes. Web: poll WebAudio context.
    ...
    return () => {
      ...
      if (hasNativeBridge) {
        ...
        window.removeEventListener("native-metrics", onNativeMetrics);
      }
    };
  ...
  // Wraps a state-mutating bridge call: while it is in flight, periodic state pushes
  // are ignored (the reply is the authoritative snapshot and is applied via
  // applyAuthoritativeState below).
  const invokeMutation = async (method, params = {}) => {
    pendingMutationsRef.current += 1;
    try {
      return await window.__nativeBridgeInvoke(method, params);
    } finally {
      pendingMutationsRef.current = Math.max(0, pendingMutationsRef.current - 1);
    }
  };

  const applyAuthoritativeState = (state) => {
    if (!state || typeof state !== "object") return;
    window.dispatchEvent(new CustomEvent("native-state", { detail: { ...state, __authoritative: true } }));
  };

  const errorClearTimerRef = useRef(0);
  const showTransientError = (message) => {
    setError(String(message || "Something went wrong"));
    if (errorClearTimerRef.current) clearTimeout(errorClearTimerRef.current);
    errorClearTimerRef.current = window.setTimeout(() => setError(""), 4000);
  };

  const surfaceRouteErrors = (state) => {
    const routeErrors = Array.isArray(state?.routeErrors) ? state.routeErrors : [];
    if (routeErrors.length > 0) {
      showTransientError(routeErrors[0]);
    }
  };
  ...
  const syncConnectionToNative = async (mode, rowId, colId, connection) => {
    ...
    if (routesPayload.length > 0) {
      try {
        const refreshed = await invokeMutation("setCrosspoints", { routes: routesPayload });
        if (syncVersion === nativeSyncVersionRef.current && refreshed) {
          surfaceRouteErrors(refreshed);
          applyAuthoritativeState(refreshed);
        }
      } catch (err) {
        // Make the failure visible and let the authoritative state (watchdog poll or
        // next push) revert the optimistic tile — never leave a silently dead route.
        showTransientError(`Route update failed: ${err?.message || err}`);
      }
    }
  };
  ...
  const buildPersistedState = (overrides = {}) => {
    // Order retention: keep offline devices' saved positions instead of persisting only
    // what is currently on screen.
    const mergedInputOrder = mergeOrder(persistedOrderRef.current.inputs, inputs.map((d) => d.deviceId));
    const mergedOutputOrder = mergeOrder(persistedOrderRef.current.outputs, outputs.map((d) => d.deviceId));
    persistedOrderRef.current = { inputs: mergedInputOrder, outputs: mergedOutputOrder };
    ...
    return {
      ...
      inputOrder: mergedInputOrder,
      outputOrder: mergedOutputOrder,
      ...overrides,
    };
  };
  ...
  const discoverDevices = async (forceRefreshNative = false, overrides = {}) => {
    try {
      ...
      // Seed order retention with the SAVED order so offline devices keep their slots.
      persistedOrderRef.current = {
        inputs: mergeOrder(savedInputOrder, orderedInputs.map((d) => d.deviceId)),
        outputs: mergeOrder(savedOutputOrder, orderedOutputs.map((d) => d.deviceId)),
      };

      // Merge with saved labels: entries for devices that are currently offline must
      // survive, or a custom label is silently wiped the first time its device is
      // unplugged during a refresh.
      const nextInputLabels = { ...(saved?.inputLabels || {}), ...createLabelMap(discoveredInputs, "Input") };
      const nextOutputLabels = { ...(saved?.outputLabels || {}), ...createLabelMap(discoveredOutputs, "Output") };
  ...
  const handleResetMatrix = async (event) => {
    ...
    if (hasNativeBridge) {
      try {
        const state = await invokeMutation("clearRoutes", {});
  ...
  useEffect(() => {
    ...
    // Debounced: a burst of tile clicks / pushes coalesces into one stringify+persist
    // instead of serializing the whole matrix on every state commit.
    if (persistDebounceRef.current) clearTimeout(persistDebounceRef.current);
    persistDebounceRef.current = window.setTimeout(() => {
      persistDebounceRef.current = 0;
      persistState({
        ...buildPersistedState(),
        matrixByView,
      });
  ...
  useEffect(() => {
    ...
    const onMatrixWheel = (event) => {
      ...
      if (tile && tile.classList.contains("on")) {
        // Wheel over an active tile adjusts its gain (handled here because tiles carry
        // no listeners of their own — see MatrixRowTiles delegation).
        event.preventDefault();
        event.stopPropagation();
        const wrapEl = target?.closest(".tile-cell-wrap[data-rowid][data-colid]");
        if (wrapEl) {
          adjustGainRef.current?.(
            wrapEl.getAttribute("data-rowid"),
            wrapEl.getAttribute("data-colid"),
  ...
  const updateConnection = (rowId, colId, updater) => {
    ...
    // Derive masters from any currently-active route when none is set.
    ...
    // The whole next matrix is computed synchronously from matrixRef (always current)
    // and set as a plain value. State updaters must stay pure — the previous version
    // ran persistence/bridge side effects inside the updater, which StrictMode
    // double-invokes and React may replay.
    const current = matrixRef.current || { device: {}, channel: {} };
    const currentModeMatrix = current[viewMode] || {};
    const currentConnection = currentModeMatrix[key] || makeDefaultConnection();
    const rawNext = typeof updater === "function" ? updater(currentConnection) : updater;
    const nextConnection = sanitizeConnection(rawNext);
    ...
    if (viewMode === "channel") {
      ...
      next.channel = convertDeviceMatrixToChannelMatrix(nextModeMatrix, current.channel || {});
    }

    const resolvedMasters = resolveActiveMasters(next.device || {}, nextInputMasterId, nextOutputMasterId);
    ...
    setMatrixByView(next);
    managerRef.current.setCrosspointGain(viewMode, rowId, colId, resolveLinearGain(nextConnection));
    // Persistence is handled by the debounced matrixByView effect.

    if (hasNativeBridge) {
      syncConnectionToNative(viewMode, rowId, colId, nextConnection).catch(() => {});
    }
  ...
  const handleToggleViewMode = () => {
    ...
    if (prevMode === "device" && nextMode === "channel") {
      const currentMatrix = matrixRef.current || { device: {}, channel: {} };
      const currentChannelMatrix = currentMatrix.channel || {};
      ...
      const nextChannelMatrix = hasCustomChannelRouting
        ...
        : convertDeviceMatrixToChannelMatrix(currentMatrix.device || {}, currentChannelMatrix);
      const nextMatrix = {
        ...currentMatrix,
      ...
      setMatrixByView(nextMatrix);
    }

    modeRef.current = nextMode;
    setViewMode(nextMode);
    // Persistence is handled by the debounced viewMode/matrixByView effect.
  };
  ...
  useEffect(() => {
    const onProgress = (event) => {
      ...
      setUpdatePercent(Number.isFinite(percent) ? clamp(Math.round(percent), 0, 100) : 0);
    };
  ...
  const handleUpdateClick = async () => {
    ...
    try {
      ...
      if (updateState === "available") {
        ...
        setUpdatePercent(0);
        await window.__nativeBridgeInvoke("updateDownload", {});
        setUpdateState("ready");
        return;
      }

      if (updateState === "ready") {
        // Host applies the update silently after our process exits, then relaunches.
        await window.__nativeBridgeInvoke("updateApply", {});
        return;
      }

      setUpdateState("checking");
      const result = await window.__nativeBridgeInvoke("updateCheck", {});
      if (result?.portable) {
        setUpdateState("portable");
        return;
      }
      if (result?.availableVersion) {
        setUpdateVersion(result.availableVersion);
        setUpdateState("available");
        return;
      }
      setUpdateState("current");
      setTimeout(() => setUpdateState((prev) => (prev === "current" ? "idle" : prev)), 2500);
    } catch (err) {
      showTransientError(`Update failed: ${err?.message || err}`);
      setUpdateState("error");
      setTimeout(() => setUpdateState((prev) => (prev === "error" ? "idle" : prev)), 3000);
    }
  };
  ...
  const updateButtonLabel = (() => {
    switch (updateState) {
      ...
      case "current": return "✓";
      case "available": return `Update v${updateVersion}`;
      case "downloading": return `${updatePercent}%`;
  ...
  const updateButtonTitle = (() => {
    switch (updateState) {
      ...
      case "available": return `Download v${updateVersion}`;
      case "downloading": return `Downloading update ${updatePercent}%`;
      case "ready": return "Restart to install the update";
      case "portable": return "Portable build — download the installer from GitHub Releases to enable in-app updates";
      case "error": return "Update check failed — click to retry";
  ...
  // The wrap-level native wheel listener (empty-dep effect) reaches the latest
  // adjustGainForCell through this ref.
  useEffect(() => {
    adjustGainRef.current = adjustGainForCell;
  });
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      invokeMutation("setCrosspoints", { routes: routesPayload })
        .then((refreshed) => {
          if (refreshed) {
            surfaceRouteErrors(refreshed);
            applyAuthoritativeState(refreshed);
        ...
        .catch((err) => {
          showTransientError(`Gain sync failed: ${err?.message || err}`);
        });
    }, 0);
  ...
  // Channel-unit lookup tables for arithmetic hover hit-testing: index i maps the i-th
  // channel-sized grid unit to its row/col id (device view spans several units per
  // device). Replaces a per-tile getBoundingClientRect sweep on every mousemove.
  const rowUnitIds = useMemo(() => {
    const arr = [];
    rows.forEach((row) => {
      const span = viewMode === "device" ? Math.max(1, row.channelCount || 1) : 1;
      for (let i = 0; i < span; i += 1) arr.push(row.id);
    });
    return arr;
  }, [rows, viewMode]);

  const colUnitIds = useMemo(() => {
    const arr = [];
    cols.forEach((col) => {
      const span = viewMode === "device" ? Math.max(1, col.channelCount || 1) : 1;
      for (let i = 0; i < span; i += 1) arr.push(col.id);
    });
    return arr;
  }, [cols, viewMode]);
  ...
  // Metrics arrive every ~100 ms; treat data as fresh for 2.5 push intervals. The old
  // threshold (captureBufferMs, default 40 ms) was SHORTER than the push interval, so
  // meters read zero for most renders and visibly flickered.
  const NATIVE_METER_FRESH_MS = 250;
  const nativeMeterDataFresh = !hasNativeBridge
    || (
      nativeMeterLastUpdateRef.current > 0
      && (performance.now() - nativeMeterLastUpdateRef.current) <= NATIVE_METER_FRESH_MS
    );
  ...
  // Delegated tile interaction: tiles themselves carry no handlers (so the memoized
  // grid rows never re-render for handler identity); the grid resolves the target tile
  // from the data attributes.
  const findTileFromEvent = (event) => {
    const wrapEl = event.target?.closest?.(".tile-cell-wrap[data-rowid][data-colid]");
    if (!wrapEl) return null;
    const rowId = wrapEl.getAttribute("data-rowid");
    const colId = wrapEl.getAttribute("data-colid");
    if (!rowId || !colId) return null;
    return { rowId, colId };
  };

  const handleGridClick = (event) => {
    const tile = findTileFromEvent(event);
    if (!tile) return;
    ...
    if (locked) return;
    const state = (matrixRef.current?.[viewMode] || {})[getCellKey(tile.rowId, tile.colId)] || makeDefaultConnection();
    updateConnection(tile.rowId, tile.colId, {
      ...state,
      on: !state.on,
    });
  };
  ...
  const handleGridMouseDown = (event) => {
    if (locked || event.button !== 1) return;
    const tile = findTileFromEvent(event);
    if (!tile) return;
    event.preventDefault();
    const state = (matrixRef.current?.[viewMode] || {})[getCellKey(tile.rowId, tile.colId)] || makeDefaultConnection();
  ...
  const handleGridContextMenu = (event) => {
    ...
    event.stopPropagation();
    if (locked) return;
    setSelectedCell({ rowId: tile.rowId, colId: tile.colId });
    updateConnection(tile.rowId, tile.colId, (prev) => ({
      ...prev,
      on: true,
          ...
          <div>
            <div className="brand-title-row">
              ...
              {hasNativeBridge ? (
                <button
                  type="button"
                  className={`brand-version-pill brand-update-btn${(updateState === "available" || updateState === "ready") ? " is-actionable" : ""}`}
        ...
        >
          <div
            ...
            onMouseMove={(event) => {
              ...
              // Arithmetic hit-testing: map the pointer to a channel-sized grid unit and
              // look up its row/col id. (The old path measured getBoundingClientRect on
              // EVERY tile per mousemove — a forced-layout storm that saturated the main
              // thread before a click could even land.)
              const unitStride = scaledCellSize + GRID_GAP_SIZE;
              const colUnit = Math.floor((x - scaledSourceWidth) / unitStride);
              const rowUnit = Math.floor((y - scaledDestinationHeight) / unitStride);
              const inside = x >= scaledSourceWidth && y >= scaledDestinationHeight
                && colUnit >= 0 && colUnit < colUnitIds.length
                && rowUnit >= 0 && rowUnit < rowUnitIds.length;

              if (!inside) {
                if (hoveredInputDeviceId) setHoveredInputDeviceId("");
                if (hoveredOutputDeviceId) setHoveredOutputDeviceId("");
                if (!locked && selectedCell) setSelectedCell(null);
                return;
              }

              const rowId = rowUnitIds[rowUnit];
              const colId = colUnitIds[colUnit];
              const rowDeviceId = rowId?.startsWith("dev:") ? rowId.slice(4) : (parseChannelId(rowId)?.deviceId || "");
              const colDeviceId = outputDeviceFromColId(colId);
              if (hoveredInputDeviceId !== rowDeviceId) setHoveredInputDeviceId(rowDeviceId);
              if (hoveredOutputDeviceId !== colDeviceId) setHoveredOutputDeviceId(colDeviceId);

              if (locked) return;
              if (dragScrollRef.current.dragging) return;
              if (isLoopbackSelfRoute(rowDeviceId, colDeviceId)) return;
            ...
            {rows.map((row, rowIndex) => {
              ...
              <React.Fragment key={row.id}>
                ...
                <MatrixRowTiles
                  ...
                  selectedColId={selectedCell?.colId || ""}
                  selectedRowPathIndex={selectedRowPathIndex}
                  selectedColPathIndex={selectedColPathIndex}
